...
class Pokemon {
  ...
  public Pokemon clone() {
    return new Pokemon(this.getId(), this.getGeneration(), this.getName(), this.getDescription(), this.getTypes(),
        this.getAbilities(), this.getWeight(), this.getHeight(), this.getCaptureRate(), this.getIsLegendary(),
        this.getCaptureDate());
  }
  ...
  public void print() {
    ...
    System.out.println("[#" + getId() + " -> " + getName() + ": " + getDescription() + " - " + strTypes + " - "
        + strAbilities + " - " + String.format("%.1f", getWeight()) + "kg - " + getHeight() + "m - " + getCaptureRate()
        + "% - " + getIsLegendary() + " - " + getGeneration() + " gen] - " + data);
  }
...
public class TP03Q10 {
    ...
    try {
      ...
      while (i <= j) {
        while ((lista.get(i).pokemon.getGeneration() < pivo.getGeneration()) ||
            (lista.get(i).pokemon.getGeneration() == pivo.getGeneration()
                && lista.get(i).pokemon.getName().compareTo(pivo.getName()) < 0)) {
          i++;
          comparacoes++;
        }
        while ((lista.get(j).pokemon.getGeneration() > pivo.getGeneration()) ||
            (lista.get(j).pokemon.getGeneration() == pivo.getGeneration()
                && lista.get(j).pokemon.getName().compareTo(pivo.getName()) > 0)) {
          j--;
          comparacoes++;
        }
        if (i <= j) {
  ...
  public static void main(String args[]) {
    ...
    try {
      resultado = Files.newBufferedWriter(Paths.get("./775799_quicksort3.txt"));
      resultado.write("775799\t" + (termino - inicio) + "\t" + comparacoes + "\t" + movimentacoes);
    } catch (IOException e) {
      e.printStackTrace();
    } finally {
      if (resultado != null) {
