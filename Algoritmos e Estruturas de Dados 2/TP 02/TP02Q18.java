...
class QuickSortParcial {
    ...
    private void quickSort(int low, int high) {
        ...
        while (i <= j) {
            while ((findPokemon[i].getGeneration() < pivot.getGeneration())
                    || ((findPokemon[i].getGeneration() == pivot.getGeneration())
                            && (findPokemon[i].getName().compareTo(pivot.getName()) < 0))) {
                i++;
                comp++;
            }

            while ((findPokemon[j].getGeneration() > pivot.getGeneration())
                    || ((findPokemon[j].getGeneration() == pivot.getGeneration())
                            && (findPokemon[j].getName().compareTo(pivot.getName()) > 0))) {
                j--;
                comp++;
            }
...
public class TP02Q18 {
    ...
    public static void main(String[] args) {
        ...
        String conteudo = "775799" + "\t" + comp + "\t" + mov + '\t' + executionTime + "ms";

        try (BufferedWriter writer = new BufferedWriter(new FileWriter("775799_partialQuickSort.txt"))) {
