/*Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo Radixsort, fazendo com que a chave de pesquisa seja o atributo abilities.
O nome do arquivo de log será matrícula_radixsort.txt.

(Lembre-se: em caso de empate, o critério de ordenação deverá ser o nome do Pokemon).*/

/*Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo da Bolha, fazendo com que a chave de pesquisa seja o atributo id.
...
// Função para obter o valor do caractere da habilidade na posição index (retorna 0 se for fim da string)
int getCharValue(char *str, int index)
{
    if (index < strlen(str))
    {
        return str[index];
...
// Função auxiliar do Radix Sort (Counting Sort)
void countingSort(Pokemon findPokemon[], int tam, int exp, int *comp, int *mov)
{
    Pokemon output[tam];
    int i, count[257] = {0}; // De 0 a 256 (256 caracteres + 1 posição para o valor 0)

    // Contagem das ocorrências dos caracteres nas habilidades
    for (i = 0; i < tam; i++)
    {
        (*comp)++;
        int charValue = getCharValue(findPokemon[i].abilities[0], exp);
        count[charValue + 1]++;
    }

    // Ajusta os índices para colocar elementos nas posições corretas
    for (i = 1; i < 257; i++)
    {
        count[i] += count[i - 1];
    }

    // Ordena com base na habilidade e, em caso de empate, compara o nome
    for (i = tam - 1; i >= 0; i--)
    {
        int charValue = getCharValue(findPokemon[i].abilities[0], exp);
        output[count[charValue + 1] - 1] = findPokemon[i];
        count[charValue + 1]--;
        (*mov)++;
    }
...
{
    // Encontra o tamanho da habilidade mais longa
    int maxLen = 0;
    for (int i = 0; i < tam; i++)
    {
        int len = strlen(findPokemon[i].abilities[0]);
        if (len > maxLen)
        {
            maxLen = len;
        }
    ...
    // Aplica Counting Sort para cada posição da habilidade
    for (int exp = maxLen - 1; exp >= 0; exp--)
    {
        countingSort(findPokemon, tam, exp, comp, mov);
    }

    // Caso as habilidades sejam iguais, ordena pelo nome
    for (int i = 0; i < tam - 1; i++)
    {
        for (int j = i + 1; j < tam; j++)
        {
            (*comp)++;
            if (strcmp(findPokemon[i].abilities[0], findPokemon[j].abilities[0]) == 0 &&
                strcmp(findPokemon[i].name, findPokemon[j].name) > 0)
            {
                swap(&findPokemon[i], &findPokemon[j]);
                (*mov)++;
            }
        }
    }
...
int main()
{
    ...
    FILE *arquivo = fopen("775799_radix.txt", "w");
    if (arquivo == NULL)
    {
