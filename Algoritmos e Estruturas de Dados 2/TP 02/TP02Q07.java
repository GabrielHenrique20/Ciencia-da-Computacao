/*Repita a questão de Ordenação por Seleção, contudo, usando o Algoritmo de Inserção, fazendo com que a chave de pesquisa seja o atributo captureDate. 
O nome do arquivo de log será matrícula_insercao.txt.
...
public class TP02Q07 {
    private static void insertionSort(Pokemon[] findPokemon, int tam, int comp, int mov) {
        for (int i = 1; i < tam; i++) {
            ...
            int j = i - 1;
            // Comparar por captureDate
            while (j >= 0 && findPokemon[j].getCaptureDate().isAfter(temp.getCaptureDate())) {
                findPokemon[j + 1] = findPokemon[j];
                j--;
                mov++;
                comp++;
            }
            findPokemon[j + 1] = temp;
            mov++;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("775799_insercao.txt"))) {
