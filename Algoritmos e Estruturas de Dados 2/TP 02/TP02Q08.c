/*Repita a questão de Ordenação por Seleção, contudo, usando o algoritmo Shellsort, fazendo com que a chave de pesquisa seja o atributo weight.
O nome do arquivo de log será matrícula_shellsort.txt.
...
int main()
{
    ...
    FILE *arquivo = fopen("775799_shellsort.txt", "w");
    if (arquivo == NULL)
    {
    ...
    fprintf(arquivo, "775799\t%d\t%%ls\t%.2f\n", comp, mov, executionTime);
    fclose(arquivo);
